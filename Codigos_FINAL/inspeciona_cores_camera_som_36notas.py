...
# -------------------------------
# Define o nome da cor mais próximo usando a partir do hex
# -------------------------------
...
# -------------------------------
# Traduz o nome de cor de inglês para português
# -------------------------------
...
# ---------------------------
# Mapeamento de frequências e notas musicais
# Considerando 36 notas e 3 escalas: 3°, 4° e 5° oitavas
# ---------------------------
NOTAS = [
    # Oitava 3
    (130, "C2"), (138, "C#2"), (146, "D2"), (155, "D#2"),
    (164, "E2"), (174, "F2"), (185, "F#2"), (196, "G2"),
    (207, "G#2"), (220, "A2"), (233, "A#2"), (246, "B2"),

    # Oitava 4
    (261, "C3"), (277, "C#3"), (293, "D3"), (311, "D#3"),
    (329, "E3"), (349, "F3"), (369, "F#3"), (392, "G3"),
    (415, "G#3"), (440, "A3"), (466, "A#3"), (494, "B3"),

    # Oitava 5
    (523, "C4"), (554, "C#4"), (587, "D4"), (622, "D#4"),
    (659, "E4"), (698, "F4"), (739, "F#4"), (784, "G4"),
    (830, "G#4"), (880, "A4"), (932, "A#4"), (987, "B4"),
]

# -------------------------------
# Converte a matiz (hue) para frequência
# -------------------------------
def hue_to_note_freq(hue):
    idx = int((hue / 360.0) * len(NOTAS))
    if idx >= len(NOTAS):
        idx = len(NOTAS) - 1
    freq, note_name = NOTAS[idx]
    return freq, note_name

# -------------------------------
# Modula e reproduz a forma de onda gerada onde:
...
def play_note_sounddevice(freq, saturation, lightness, dur):
    ...
    # Cria onda final, somando a onda base com os harmônicos.
    # A saturação é inserida para modificar as intensidades de cada onda
    wave = (
        base * (1.0 - saturation)
    ...
    # Insere a luminosidade para controlar a amplitude da onda final
    # Evita silêncio absoluto e clipping
    amplitude = 0.05 + 0.4 * lightness
    wave *= amplitude

    #Reproduz a onda sonora criada
    sd.play(wave, sr)
    sd.wait()
...
# -------------------------------
# Gera e exibe gráficos da onda base, harmônicas e onda final do som reproduzido
# -------------------------------
def plot_graphs(freq, hue, saturation, lightness, note_name):
    # Fecha qualquer janela de gráfico já aberta
    plt.close('all')
    ...
    #Define o título da página com os fatores usados no cálculo
    fig.suptitle(
        f"Nota: {note_name}   |   Frequência: {freq:.1f} Hz\n"
        f"H: {hue:.1f}°   S: {saturation:.2f}   L: {lightness:.2f}",
        fontsize=13,
        fontweight="bold"
    )

    # Mostrar ao máximo, 10 períodos da maior frequência mapeada
    t = np.linspace(0, 10/NOTAS[len(NOTAS)-1][0], 500)

    base = np.sin(2 * np.pi * freq * t) * (1.0 - saturation)
    harmonic2 = np.sin(2 * np.pi * freq * 2 * t) * (0.6 * saturation)
    harmonic3 = np.sin(2 * np.pi * freq * 3 * t) * (0.3 * saturation)

    wave = base + harmonic2 + harmonic3
    wave *= (0.05 + 0.4 * lightness)

    # ----- gráfico A: componentes -----
    axd['A'].plot(t, base, label='Onda Base', color='green')
    axd['A'].plot(t, harmonic2, label='2ª Harmônica', color='orange', linestyle='dashed')
    axd['A'].plot(t, harmonic3, label='3ª Harmônica', color='purple', linestyle='dashdot')

    axd['A'].set_title("Onda base e harmônicas")
    axd['A'].set_xlabel("Tempo (s)")
    axd['A'].set_ylabel("Amplitude")
    axd['A'].legend()

    # ----- gráfico B: onda final -----
    axd['B'].plot(t, wave, label='Onda Sonora', color='blue')
    axd['B'].set_title("Onda sonora resultante")
    axd['B'].set_xlabel("Tempo (s)")
    axd['B'].set_ylabel("Amplitude")
...
# -------------------------------
# Dá os comandos de reproduzir o som e gerar os gráficos
# -------------------------------
def play_color_sound(bgr):
    h, s, l = bgr_to_hsl(bgr)
    freq, _ = hue_to_note_freq(h)
    play_note_sounddevice(freq, s, l, dur=1)
    return

# -------------------------------
# Define as ações a serem executadas com a movimentação do mouse
# -------------------------------
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        if 0 <= x < w and 0 <= y < h:
            ...
            if now - last_click_time > 0.1:
                ...
                h, s, l = bgr_to_hsl(bgr)
                freq, note_name = hue_to_note_freq(h)

                plot_graphs(freq, h, s, l, note_name)
