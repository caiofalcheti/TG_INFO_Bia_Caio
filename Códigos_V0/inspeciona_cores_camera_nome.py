...
def sem_acentos(s: str) -> str:
    """Remove acentos para poder desenhar com cv2.putText (fonte ASCII)."""
    if s is None:
        return ""
    # Normaliza para NFD e remove caracteres de marca (acentos)
    nf = unicodedata.normalize("NFD", s)
    return "".join(c for c in nf if unicodedata.category(c) != "Mn")

# Tentativa de importar winsound (Windows)
try:
    import winsound
    HAS_WINSOUND = True
except ImportError:
    HAS_WINSOUND = False
    print("winsound não disponível - som será apenas simulado no console.")
...
def translate_color_name(en_name: str) -> str:
    """
    Tradução aproximada: tenta primeiro um dicionário de casos especiais
    e depois quebra o nome em partes para traduzir componentes.
    """
    if en_name is None:
        return "desconhecida"
    ...
    # 1) CASOS ESPECIAIS (traduz direitinho os nomes mais comuns/compostos)
    especiais = {
        ...
        "honeydew": "melão-de-mel (honeydew)",
    ...
    # 2) COMPONENTES BÁSICOS E MODIFICADORES
    COMPONENT_PT = {
        "red": "vermelho",
        "green": "verde",
        "blue": "azul",
    ...
    # 3) Tenta decompor o nome em "pedaços" conhecidos
    tokens = []
    i = 0
    all_keys = list(MODIFIER_PT.keys()) + list(COMPONENT_PT.keys())
    # ordenar por tamanho decrescente para pegar palavras maiores primeiro
    ...
    while i < len(name):
        ...
        for k in all_keys:
            if name.startswith(k, i):
                ...
                break
        if not matched:
            i += 1  # ignora caracteres que não fazem parte de nenhuma chave

    if not tokens:
        # se não conseguiu decompor, devolve o nome original
        return en_name
...
# ---------------------------
# Mapeia cor → frequência Hz
# ---------------------------
def bgr_to_hex(bgr):
    b, g, r = int(bgr[0]), int(bgr[1]), int(bgr[2])
    return f"#{r:02X}{g:02X}{b:02X}"

def color_to_frequency(bgr):
    """
    Mapeia o matiz (H) da cor em HSV para uma frequência audível.
    H (0–179 no OpenCV) → 220–1760 Hz aproximadamente.
    """
    ...
    freq = f_min + (h / 179.0) * (f_max - f_min)
    return int(freq)

def play_color_sound(bgr):
    freq = color_to_frequency(bgr)
    dur = 200  # duração em ms
    if HAS_WINSOUND:
        winsound.Beep(freq, dur)
    else:
        print(f"(som) frequência ~ {freq} Hz")
...
def on_mouse(event, x, y, flags, userdata):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        if 0 <= x < w and 0 <= y < h:
            bgr = frame_atual[y, x]
            # pequeno debounce para não tocar mil vezes sem querer
            agora = time.time()
...
def main():
    ...
    while True:
        ...
        if mouse_inside:
            ...
            # círculo na posição do pixel
            cv2.circle(frame, (x, y), 4, (0, 255, 0), 1)

            # textos
            cv2.putText(placeholder, f"Pos: x={x} y={y}", (20, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
            cv2.putText(placeholder, f"BGR: {bgr.tolist()}", (20, 80),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
            cv2.putText(placeholder, f"HSV: {hsv.tolist()}", (20, 120),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
            cv2.putText(placeholder, f"HEX: {hex_code}", (20, 160),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 1)
            cv2.putText(placeholder, f"Nome (EN): {name_en}", (20, 200),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 1)
            cv2.putText(placeholder,f"Nome (PT): {sem_acentos(name_pt)}",(20, 240),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 1,)
            cv2.putText(placeholder, "Clique = tocar som", (20, 280),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)

            # Paleta/bloco de cor (retângulo preenchido)
            cv2.rectangle(placeholder, (20, 310), (380, 380),
